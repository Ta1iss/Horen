...
\nchapter{Числа}
\noindent Язык На'ви имеет восьмеричную систему счисления \textit{octal}, основанную на числе восемь. Как в немногих человеческих языках.\footnote{Видимо результат пересчета не пальцев, а промежутков между ними.}  Вместо подсчёта чисел по десятеричной формуле $(m \times 10) + n$ (как в $(4 \times 10) + 2 = 42_{10}$,
...
изгоняют стоящую на конце \N{-l} из названий ``восьмятков''.  Подобным образом, последняя \N{-m} в формах \N{zam, vozam,} и \N{zazam} убирается перед всем, кроме ``единицы,'' \N{zamaw,} но \N{za\ACC{mun},
za\ACC{pey}}, и т.д.

\subsubsection{} Ударение получают присоединяемые зависимые формы.
...
\noindent Эта закономерность продолжится с \N{\ACC{me}vol}:
...
\subsection{Суффикс -ve} Порядковые числительные образуются с помощью суффикса \N{-ve}, который не меняет на ударение в слове, хотя приводит к изменению некоторых корней чисел. \index{-ve@\textbf{-ve}}
\index{numbers!ordinal}

\begin{center}
\begin{tabular}{rll}
Порядковая & Независимая & Зависимая \\
\hline
...
\subsubsection{} Порядковые числительные применяются как прилагательные, получая маркер \N{-a-}, если они использованы как определение (\horenref{morph:adj-attr}). Например, \Npawl{mrrvea ikran} \E{пятый икран}.
\LNForum{27/1/2021}{https://forum.learnnavi.org/language-updates/cardinal-zam-vozam-and-zazam/}
...
\subsection{-Pxì} За исключением \E{половины} и \E{трети}, у которых есть свои названия, дроби образуются с помощью замены \N{-ve} на \N{-pxì} в порядковом числительном.  Обратите вни\-ма\-ние на изменение ударения:
...
одна пятая & \N{mrr\ACC{pxì}} \\
\end{tabular}
\hskip2em
\begin{tabular}{rl}
одна шестая & \N{pu\ACC{pxì}} \\
одна седьмая & \N{ki\ACC{pxì}} \\
одна восьмая & \N{vo\ACC{pxì}} \\
\\
\end{tabular}
\end{center}
...
\subsubsection{Две трети} Дробь \E{две трети} имеет отдельную форму, \N{mefan}. Двойственное для \N{pan}. \index{mefan@\textbf{mefan}}
...
А все остальные образуются с помощью связки с прилагательным, \Npawl{\uwave{alo
amrr} poan polawm} \E{она спросила \uwave{пять раз}}. \index{melo@\textbf{melo}}
...
\N{'eyt} для \E{восьми} и \N{nayn} для \E{девяти}. Их не используются для подсчета, а только для таких вещей как, напнрмер, номера телефонов.
\index{'eyt@\textbf{'eyt}}\index{nayn@\textbf{nayn}}

\subsubsection{} \N{Kew} это \E{ноль}.  \QUAESTIO{Текущая документация не уточняет, является ли это понятие род\-ным или заимствованным у людей.}
\index{kew@\textbf{kew}}
